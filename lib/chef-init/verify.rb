  ...
  class Verify
    ...
    def run
      ...
      if ChefInit::Test.did_pass?
        ...
        failed_test_string = ChefInit::Test.failed_tests.map { |t| "\t{t}" }.join("\n")
    ...
    def run_onboot_tests
      ...
      system_command("#{omnibus_bin_dir}/chef-init --onboot -c #{tempdir}/zero.rb -j #{tempdir}/first-boot.json")
    ...
    def setup_test_environment
      ...
      File.open(File.join(tempdir, 'zero.rb')) do |f|
        f.write(<<-ZERO_CONFIG
require 'chef-init'

cookbooks_path   ["#{tempdir}/cookbooks"]
ssl_verify_mode   :verify_peer
        ZERO_CONFIG
        )
      end
      File.open(File.join(tempdir, 'first-boot.json')) do |f|
        f.write(<<-FIRST_BOOT
{
  ...
  "container_service": {
    "polipo": {
      ...
      test_cookbook_path = File.join(tempdir, 'cookbooks', 'test', 'recipes')
      File.mkdir_p(test_cookbook_path)
      File.open(File.join(test_cookbook_path, 'metadata.rb')) do |f|
        f.write('name "test"')
      end

      File.open(File.join(test_cookbook_path, 'recipes', 'default.rb')) do |f|
        f.write(<<-RECIPE
package 'polipo' do
  action :install
end
